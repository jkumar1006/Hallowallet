...
export async function POST(req: NextRequest) {
  try {
    ...
    // Disposable Income = Income - Savings Target - Expected Monthly Expenses (from profile)
    const disposableIncome = profile.monthlyIncome - monthlySavingsTarget - expectedMonthlyExpenses;
    const remainingBudget = disposableIncome;
    // Yearly disposable = (Income - Savings Target - Expected Expenses) * 12
    ...
    const itemAnalyses = items.map((item: any) => {
      ...
      if (timePeriod === "monthly") {
        if (afterPurchaseBalance > disposableIncome * 0.2) {
          recommendation = "buy";
          reason = `✅ Monthly: Comfortable. $${afterPurchaseBalance.toFixed(2)} left this month.`;
        } else if (afterPurchaseBalance > 0) {
          recommendation = "buy";
          reason = `🟡 Monthly: Tight. $${afterPurchaseBalance.toFixed(2)} left this month.`;
        } else if (canAffordFromYearlyDisposable && yearlyDisposableAfterPurchase > yearlyDisposableIncome * 0.3) {
          recommendation = "wait";
          reason = `❌ Monthly: Exceeds budget by $${Math.abs(afterPurchaseBalance).toFixed(2)}.\n💡 Wait or use yearly budget.`;
        } else if (canAffordFromYearlyDisposable) {
          recommendation = "reconsider";
          reason = `❌ Monthly: Exceeds budget by $${Math.abs(afterPurchaseBalance).toFixed(2)}.\n⚠️ Uses most yearly disposable.`;
        } else {
          recommendation = "reconsider";
          reason = `❌ Monthly: Exceeds budget by $${Math.abs(afterPurchaseBalance).toFixed(2)}.\n🚫 Cuts $${amountCutFromSavings.toFixed(2)} from savings.`;
        }
      } else {
        // Yearly analysis
        if (canAffordFromYearlyDisposable && yearlyDisposableAfterPurchase > yearlyDisposableIncome * 0.5) {
          recommendation = "buy";
          reason = `✅ Yearly: Affordable. $${yearlyDisposableAfterPurchase.toFixed(2)} left for year.`;
        } else if (canAffordFromYearlyDisposable && yearlyDisposableAfterPurchase > yearlyDisposableIncome * 0.3) {
          recommendation = "buy";
          reason = `🟡 Yearly: Affordable but uses significant budget. $${yearlyDisposableAfterPurchase.toFixed(2)} left.`;
        } else if (canAffordFromYearlyDisposable) {
          recommendation = "reconsider";
          reason = `⚠️ Yearly: Uses most disposable income. Only $${yearlyDisposableAfterPurchase.toFixed(2)} left.`;
        } else {
          recommendation = "reconsider";
          reason = `🚫 Yearly: Exceeds disposable by $${amountCutFromSavings.toFixed(2)}. Cuts into savings.`;
        }
      }
    ...
    return NextResponse.json({
      ...
      itemPrice: bestItem.itemPrice,
      impactOnSavings: bestItem.afterPurchaseBalance,
      remainingBudget,
      afterPurchaseBalance: bestItem.afterPurchaseBalance,
      alternativeSuggestions: bestItem.alternativeSuggestions,
    });
  } catch (error) {
